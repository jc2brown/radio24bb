        ...
        while True:
            ...
            if transferred > 0:
                ...
                # for d in data[0:0]:
                    ...
                    #     sys.stdout.write('\n')
                    # elif ord(d) != 0:
                    #     sys.stdout.write(d.decode("latin_1"))
            ...
            if transferred != size:
                ...
                status = D3XX.getLastError()
                if (status != 0):
                    logging.debug("Read[%#04X] error status %d (%s)" % (pipe, status, ftd3xx.getStrError(status)))
                    CancelPipe(D3XX, pipe)
                    # result = False
            ...
            continue
                ...
                # for i in range(0, transferred, 4):
                #     d = 0
...
def ChannelThreadFunc(D3XX, channel, size, iteration, output, timeout, bWrite, bRead):
    ...
    #dataBuffer[:] = itertools.repeat(0xFF, len(dataBuffer))

    for i in range(len(dataBuffer)):
        dataBuffer[i] = (i // 4) % 256
        #dataBuffer[i] = random.getrandbits()
    #dataBuffer = [random.getrandbits(32) for i in range(len(dataBuffer))]
    #dataBuffer = [0xAA for i in range(len(dataBuffer))]
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename='datastreaming.log', filemode='w', level=logging.DEBUG, format='[%(asctime)s] %(message)s')
