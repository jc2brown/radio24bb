
#include "xil_printf.h"
#include <stdio.h>
#include "sleep.h"
#include "radio24bb.h"

#include "command.h"
#include "usb.h"





char sin_table[1024] = {
	,0xf,
	0xf,0xf,0xe,0xe,0xe,0xd,0xd,0xd,0xc,0xc,0xc,0xb,0xb,0xb,0xa,0xa,
	0xa,0x9,0x9,0x9,0x9,0x8,0x8,0x8,0x7,0x7,0x7,0x7,0x6,0x6,0x6,0x6,
	0x5,0x5,0x5,0x5,0x5,0x4,0x4,0x4,0x4,0x4,0x3,0x3,0x3,0x3,0x3,0x3,
	0x2,0x2,0x2,0x2,0x2,0x2,0x2,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,
	0x1,0x1,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,
	0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1,
	0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x2,0x2,0x2,0x2,0x2,0x2,
	0x2,0x3,0x3,0x3,0x3,0x3,0x3,0x4,0x4,0x4,0x4,0x4,0x5,0x5,0x5,0x5,
	0x5,0x6,0x6,0x6,0x6,0x7,0x7,0x7,0x7,0x8,0x8,0x8,0x9,0x9,0x9,0x9,
	0xa,0xa,0xa,0xb,0xb,0xb,0xc,0xc,0xc,0xd,0xd,0xd,0xe,0xe,0xe,0xf,
	0xf,0xf,
};







void handle_usb_src_cmd(void *arg, struct command *cmd) {
	struct radio24bb *r24bb = (struct radio24bb *)arg;
	static char *usb_srcs[] = { "raw", "zero", "ina", "loopback" };
	char *src = cmd->tokens[cmd->index++];
	if (!strcmp(src, "help")) {
		for (int i = 0; i < sizeof(usb_srcs)/sizeof(*usb_srcs); ++i) {
			xil_printf("%d:%s  ", i, usb_srcs[i]);
		}
		xil_printf("\n");
		return;
	}
	for (int i = 0; i < sizeof(usb_srcs)/sizeof(*usb_srcs); ++i) {
		if (!strcmp(src, usb_srcs[i])) {
			r24bb->regs->usb_wr_mux = i;
		}
	}
}





void handle_usb_atrig_cmd(void *arg, struct command *cmd) {
	struct radio24bb *r24bb = (struct radio24bb *)arg;
	char *atrig_str = cmd->tokens[cmd->index++];
	char *endptr;
	unsigned long atrig = strtoul(atrig_str, &endptr, 0);
	if (atrig == 0 && atrig_str == endptr) {
		return;
	}
	for (int i = 0; i < 1000; ++i) {
		r24bb->regs->ina_usb_wr_count = atrig;
		usleep(10000);
	}
} 





// Set or clear USB RESET_N
//
int set_usb_reset_n(struct radio24bb *r24bb, int reset_n) {
	// Critical section
	// - need to guard bus_sel (modified by ioexp_read_port  shared by usb_ioexp_0 and usb_ioexp_1
	//Xil_ExceptionDisable(); 

	int ireg = mfcpsr();
	Xil_ExceptionDisable();

	u8 ioe1_port0_value;
	ioexp_read_port(r24bb->usb_ioexp_1, 0, &ioe1_port0_value);

	if (reset_n == 0) {
		ioe1_port0_value &= ~USB_IOEXP_1_USB_RESET_N;
	} else {
		ioe1_port0_value |= USB_IOEXP_1_USB_RESET_N;
	}

	ioexp_write_port(r24bb->usb_ioexp_1, 0, ioe1_port0_value);

	// Xil_ExceptionEnable();
	mtcpsr(ireg);
	return XST_SUCCESS;
}







void handle_usb_reset_cmd(void *arg, struct command *cmd) {
	struct radio24bb *r24bb = (struct radio24bb *)arg;
	char *reset_str = cmd->tokens[cmd->index++];
	char *endptr;
	unsigned long reset = strtoul(reset_str, &endptr, 0);
	if (reset == 0 && reset_str == endptr) {
		return;
	}
	if (reset == 2) {
		set_usb_reset_n(r24bb, 0);
		usleep(1000000);
		set_usb_reset_n(r24bb, 1);
	}
	if (reset == 1) {
		set_usb_reset_n(r24bb, 0);
	}
	if (reset == 0) {
		set_usb_reset_n(r24bb, 1);
	}
} 






void send_usb_hello(void *arg, struct command *cmd) {
	struct radio24bb *r24bb = (struct radio24bb *)arg;

	static int n = 0;
	static char msg[1024];

	sprintf(msg, "Hello #%d\n", n++);
	for (char *c = msg; *c != '\0'; ++c) {
		r24bb->regs->usb_wr_data = *c;
	}
	//usleep(100ULL);
	r24bb->regs->usb_wr_push = 1;

}






void send_usb_sine(void *arg, struct command *cmd) {
	struct radio24bb *r24bb = (struct radio24bb *)arg;

	int phase = random() % 1024;
	for (int i = 0; i < 1024; ++i) {
		r24bb->regs->usb_wr_data = sin_table[phase%1024]-(char)128;
		phase += 11;
	}
	r24bb->regs->usb_wr_push = 1;
	//r24bb->regs->usb_wr_push = 1;

}













void init_usb_channel_context(char *name, void* arg, struct cmd_context *parent_ctx) {

	struct cmd_context *usb_channel_ctx = make_cmd_context(name, arg);
	add_subcontext(parent_ctx, usb_channel_ctx);
	add_command(usb_channel_ctx, "src", handle_usb_src_cmd);
	add_command(usb_channel_ctx, "hello", send_usb_hello);
	add_command(usb_channel_ctx, "sine", send_usb_sine);
	add_command(usb_channel_ctx, "atrig", handle_usb_atrig_cmd);
	add_command(usb_channel_ctx, "reset", handle_usb_reset_cmd);

}








